...
func Test_SimpleRetryWithOverloadErr(t *testing.T) {
	ctx := context.Background()
	definedErr := errors.New("err from outputErr")
	var retryTimes int
	err := goretry.Do(ctx, func() error {
		retryTimes++
		if retryTimes == 1 {
			// downstream overload, should stop
			return goretry.ErrorOverload
		}
		return definedErr
	})
	if err == nil {
	...
	if err != goretry.ErrorOverload {
		t.Errorf("returned err is not overload, err=%v", err)
	}
	if retryTimes != 1 {
		t.Errorf("retry times not expected")
	}
